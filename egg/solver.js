...
function isValidMove(note) {
    const size = note[0].toUpperCase() === 'L' ? 3 : (note[0].toUpperCase() === 'M' ? 2 : 1);
    const pos = parseInt(note[1])-1;

    const currentPlayer = getPlayerTurn(p1, p2);
    if (currentPlayer[size] === 0) return false; // no more eggs of this size
    if (boardPlay[pos] === currentPlayer.type) return false; // cannot replace your own egg
    if (boardSize[pos] >= size) return false; // egg insufficient size to replace
